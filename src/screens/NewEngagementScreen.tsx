      ...
      <div className="max-w-2xl">
        ...
        {mode === 'new' ? (
          <NewOpportunityCapture
            ...
            selectPhoto={(event) => selectPhoto(event, 'new')}
          />
        ) : (
          <ExistingWorkCapture
            ...
            setExistingCapture={setExistingCapture}
            existingPhotoFile={existingPhotoFile}
            setExistingPhotoFile={setExistingPhotoFile}
            selectPhoto={(event) => selectPhoto(event, 'existing')}
            mode={existingMode}
            setMode={setExistingMode}
            factLabel={factLabel}
            setFactLabel={setFactLabel}
            factCategory={factCategory}
            setFactCategory={setFactCategory}
            factKind={factKind}
            setFactKind={setFactKind}
            dueAt={dueAt}
            setDueAt={setDueAt}
          />
        )}
        ...
        <button type="button" onClick={() => void (mode === 'new' ? submitNew() : submitExisting())} disabled={saving} className="mt-6 w-full rounded-xl bg-amber-500 px-5 py-4 font-bold text-zinc-950 disabled:opacity-50">{saving ? 'Preserving…' : mode === 'new' ? 'Capture new opportunity' : 'Capture update'}</button>
      </div>
    </div>
  )
}
...
function NewOpportunityCapture({
  ...
  return <>
    ...
    <div className="mt-2 rounded-xl border border-zinc-900 bg-zinc-950/60 p-3 text-xs leading-5 text-zinc-600">This note stays source evidence. It is not automatically treated as the customer’s literal request or as verified technical truth. Native AI interpretation is still intentionally outside canonical authority.</div>
    ...
    <details className="mt-6 rounded-2xl border border-zinc-900 bg-zinc-950/50 p-4">
      ...
      <div className="mt-5 grid gap-4">
        <label className="text-sm text-zinc-400">Engagement / project name<input value={name} onChange={(event) => setName(event.target.value)} className="mt-2 w-full rounded-xl border border-zinc-800 bg-zinc-950 px-4 py-3 outline-none focus:border-amber-600" placeholder="Cooper River Bridge Run 2027" /></label>
        <label className="text-sm text-zinc-400">Type<select value={engagementType} onChange={(event) => setEngagementType(event.target.value as EngagementType)} className="mt-2 w-full rounded-xl border border-zinc-800 bg-zinc-950 px-4 py-3 text-zinc-100 outline-none focus:border-amber-600"><option value="OTHER">Other / Unsure</option><option value="EVENT">Event</option><option value="LONG_TERM_RENTAL">Long-Term Rental</option><option value="INSTALLATION">Installation</option><option value="EQUIPMENT_SALE">Equipment Sale</option><option value="SERVICE">Service</option></select></label>
        <label className="text-sm text-zinc-400">Customer asked for<textarea value={request} onChange={(event) => setRequest(event.target.value)} rows={3} className="mt-2 w-full rounded-xl border border-zinc-800 bg-zinc-950 px-4 py-3 outline-none focus:border-amber-600" placeholder="Only the customer’s request, if you know it clearly" /></label>
        <div className="grid gap-4 sm:grid-cols-2">
          <label className="text-sm text-zinc-400">Event / project date<input type="date" value={eventDate} onChange={(event) => setEventDate(event.target.value)} className="mt-2 w-full rounded-xl border border-zinc-800 bg-zinc-950 px-4 py-3 text-zinc-100 outline-none focus:border-amber-600" /><span className="mt-1 block text-xs leading-5 text-zinc-600">Date-only is valid. Do not invent a time just to complete the record.</span></label>
          <label className="text-sm text-zinc-400">Venue / location<input value={venueName} onChange={(event) => setVenueName(event.target.value)} className="mt-2 w-full rounded-xl border border-zinc-800 bg-zinc-950 px-4 py-3 outline-none focus:border-amber-600" placeholder="Hotel Bennett" /></label>
        </div>
        <label className="text-sm text-zinc-400">Next move<input value={nextAction} onChange={(event) => setNextAction(event.target.value)} className="mt-2 w-full rounded-xl border border-zinc-800 bg-zinc-950 px-4 py-3 outline-none focus:border-amber-600" placeholder="Only add this if the next action is actually known" /></label>
      </div>
    </details>
  </>
}
...
function ExistingWorkCapture({
  ...
  return <>
    ...
    <select value={selectedEngagementId} onChange={(event) => setSelectedEngagementId(event.target.value)} disabled={engagementsLoading} className="mt-2 w-full rounded-xl border border-zinc-800 bg-zinc-950 px-4 py-3 text-zinc-100 outline-none focus:border-amber-600 disabled:opacity-50">
    ...
    {(mode === 'FACT' || mode === 'UNKNOWN') && (
      <div className="mt-4 grid gap-3 rounded-2xl border border-zinc-900 bg-zinc-950/50 p-4">
        <label className="text-sm text-zinc-400">Short label<input value={factLabel} onChange={(event) => setFactLabel(event.target.value)} placeholder={mode === 'UNKNOWN' ? 'Need confirmed load-in time' : 'Venue power available'} className="mt-2 w-full rounded-xl border border-zinc-800 bg-zinc-950 px-4 py-3 outline-none focus:border-amber-600" /></label>
        <div className="grid gap-3 sm:grid-cols-2">
          <label className="text-sm text-zinc-400">Domain<select value={factCategory} onChange={(event) => setFactCategory(event.target.value as FactCategory)} className="mt-2 w-full rounded-xl border border-zinc-800 bg-zinc-950 px-3 py-2.5"><option>EVENT</option><option>VISUAL</option><option>AUDIO</option><option>LIGHTING</option><option>STAGING</option><option>POWER</option><option>NETWORK</option><option>VENUE</option><option>LOGISTICS</option><option>LABOR</option><option>CONTENT</option><option>CUSTOMER</option><option>OTHER</option></select></label>
          <label className="text-sm text-zinc-400">Kind<select value={factKind} onChange={(event) => setFactKind(event.target.value as FactKind)} className="mt-2 w-full rounded-xl border border-zinc-800 bg-zinc-950 px-3 py-2.5"><option>REQUIREMENT</option><option>CONSTRAINT</option><option>CUSTOMER_REQUEST</option><option>OBSERVATION</option><option>ASSUMPTION</option><option>PREFERENCE</option><option>OTHER</option></select></label>
        </div>
    ...
    {mode === 'NEXT_MOVE' && (
      <label className="mt-4 block text-sm text-zinc-400">Due / follow-up <span className="text-zinc-600">optional</span><input value={dueAt} onChange={(event) => setDueAt(event.target.value)} type="datetime-local" className="mt-2 w-full rounded-xl border border-zinc-800 bg-zinc-950 px-4 py-3 text-zinc-100" /></label>
    )}
