  ...
  return (
    <div className="sm:ml-48">
      ...
      <div className="max-w-2xl">
        <p className="text-sm text-zinc-500">Tell Stage Presence what happened once. The source stays intact; structure can follow.</p>
        ...
        <div className="mt-2 rounded-xl border border-zinc-900 bg-zinc-950/60 p-3 text-xs leading-5 text-zinc-600">
          This note stays source evidence. It is not automatically treated as the customer’s literal request or as verified technical truth. No AI interpretation is connected yet.
        </div>
        ...
        <details className="mt-6 rounded-2xl border border-zinc-900 bg-zinc-950/50 p-4">
          ...
          <div className="mt-5 grid gap-4">
            ...
            <label className="text-sm text-zinc-400">Customer asked for
              <textarea value={request} onChange={(event) => setRequest(event.target.value)} rows={3} className="mt-2 w-full rounded-xl border border-zinc-800 bg-zinc-950 px-4 py-3 outline-none focus:border-amber-600" placeholder="Only the customer’s request, if you know it clearly" />
            </label>

            <div className="grid gap-4 sm:grid-cols-2">
              <label className="text-sm text-zinc-400">Event / project date
                <input type="date" value={eventDate} onChange={(event) => setEventDate(event.target.value)} className="mt-2 w-full rounded-xl border border-zinc-800 bg-zinc-950 px-4 py-3 text-zinc-100 outline-none focus:border-amber-600" />
                <span className="mt-1 block text-xs leading-5 text-zinc-600">Date-only is valid. Do not invent a time just to complete the record.</span>
              </label>
            ...
            <label className="text-sm text-zinc-400">Next move
              <input value={nextAction} onChange={(event) => setNextAction(event.target.value)} className="mt-2 w-full rounded-xl border border-zinc-800 bg-zinc-950 px-4 py-3 outline-none focus:border-amber-600" placeholder="Only add this if the next action is actually known" />
            </label>
          </div>
        </details>
