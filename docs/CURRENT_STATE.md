...
### Stage Presence MCP — OAuth-backed Supabase Edge Function, 26 tools (2026-09-21)
...
16 read tools: `find_contact`, `find_engagement`, `get_pricing`, `generate_lead_summary`, `generate_job_sheet` (an operational projection), `search_stage_presence`, `get_attention_items`, `get_upcoming_engagements`, `build_quote_draft` (proposes but never persists a quote), `find_team_member`, `find_resource`, `get_capabilities` (truthful `READ`/`DRAFT_ONLY`/`APPROVAL_REQUIRED`/`FRONTIER`/`NOT_IMPLEMENTED` list), `get_pricing_review_queue` (groups the Price Book by `authority_state` with a plain-language reason each item isn't approved authority), `check_team_conflicts` (never reports AVAILABLE — only known-conflicting/known-scheduled/no-conflict-found/insufficient-evidence), `check_resource_pressure` (never reports AVAILABLE, never creates a hold — compares demand to configured quantity only when it's `VERIFIED`), `generate_daily_brief` (synthesizes the above into an owner/PM view, every line tagged FACT/DERIVED_ATTENTION/UNKNOWN). Plus `generate_email` (draft-only, `readOnlyHint: true` — never sends, never changes engagement state).

9 write tools, all requiring `confirmed: true` and a caller-supplied `idempotencyKey`, all performing a canonical re-read before reporting `VERIFIED_SAVED` (never `SAVED` from the write response alone), all refusing idempotency-key reuse with a changed payload:
- `set_next_action` — creates exactly one `OPEN` `work_items` row per approved proposal.
...
- `create_lead` — the first real intake path: writes one `engagements` row (a lead is not a separate entity) plus, only where supplied, a linked contact/venue and known-unknown facts, reusing `src/lib/canonicalWrites.ts`'s existing ambiguous-match-never-guessed precedent for contacts/venues. Idempotent via `engagements.source_key` (already reserved for "idempotent external-system imports").
- `save_quote_draft` — persists a `DRAFT` `commercial_documents` quote, faithfully porting `src/lib/pricingRuntime.ts`'s already-proven validation/calculation rules (retired/DRAFT rule gating, effective-date windows, price-adjustment-reason requirement, totals formula). Never promotes a `DRAFT_CANDIDATE` Price Book rate to approved policy, and validates every line before writing anything so one bad line never leaves a partial quote.
- `assign_team_member` — writes `engagement_assignments`, but the schema itself only permits `POSSIBLE`/`REQUESTED` — `CONFIRMED` is refused outright, since nothing in the schema evidences actual availability yet. Always reports that the assignment_state does not prove availability, and now includes `conflictEvidence` from `check_team_conflicts`.
- `add_resource_requirement` — writes only `engagement_resources` (`relationship`/requirement-window/sourcing fields already established by the schema); never touches `resource_commitments` (the real HOLD/RESERVATION/ALLOCATION table, confirmed to exist cleanly but deliberately left untouched). Always reports that this does not mean the resource is available, held, or reserved.
- `approve_pricing_rule` — the one path from `DRAFT` to `APPROVED_AUTHORITY`. Writes only `status`/`approved_by`/`approved_at` (the schema's own approval fields) — never amount/percentage/scope. Requires the caller's last-read `updated_at` and refuses (both in application logic and via a second `.eq('updated_at', ...)` guard on the write) if the rule changed since then. Only reports `VERIFIED_SAVED` after re-reading `price_book_v` and confirming `authority_state = 'APPROVED_AUTHORITY'`.
- `update_pricing_rule_draft` — edits one `DRAFT` rule's name/amount/percentage/notes/rationale/effective window; refuses outright on any non-`DRAFT` rule.

**Proven live**: on 2026-09-21, Greg approved and `set_next_action` created a real work item (`20d10a27-e4fa-4a6b-ae38-d5447ef4186d`, "Follow up on final LowVelo IOP scope") on `SP-000014`, confirmed `VERIFIED_SAVED` by canonical re-read, and `engagement_summary_v.next_work` reflected it correctly. All other write tools are built and contract-tested but not yet exercised against live business data.

**Deployed**: server version `1.5.0` is live, confirmed byte-identical to `main` by re-fetching the deployed source and diffing/hashing it against the repo file (`sha256` match) immediately after deploy.

**True frontiers still deliberately not crossed**: confirming crew availability (no acknowledgement/calendar evidence source), creating a resource `HOLD`/`RESERVATION` (the `CONFIGURED → REQUIREMENT WINDOW → PRESSURE → HOLD → RESERVATION` policy isn't built — `check_resource_pressure` is the evidence layer for it), and creating a brand-new pricing rule from scratch via AI (approving/editing an *existing* `DRAFT` rule is no longer a frontier).

**`build_quote_draft`'s read logic proven against real data (2026-09-21)**: the calling session's connected MCP tool list is a fixed snapshot taken when it connected and does not yet include `build_quote_draft` (or the other tools added after that snapshot), so the tool itself could not be invoked directly this session. Its exact read-only query logic (match each `engagement_resources` row's resource name against `price_book_v` via `ilike`, prefer `authority_state = 'APPROVED_AUTHORITY'`, else the next candidate) was instead run directly as SQL against the live database — a faithful line-for-line replay of `buildQuoteDraft()`'s own matching logic, not a re-derivation. Result against `SP-000030` ("Beerfest 2026", 15 linked resources, the most of any engagement): 14 of 15 resources matched no priced rule at all (`needsPrice: true`); one (`Load-in`) matched a `DRAFT_CANDIDATE` flat $200 rule. A system-wide check confirmed **zero rows in `price_book_v` currently carry `authority_state = 'APPROVED_AUTHORITY'`** — so no engagement anywhere in the system can currently produce an `APPROVED_AUTHORITY` quote line, which `build_quote_draft` would (and, per this check, correctly does) report truthfully rather than fabricate. The tool itself still needs a direct in-session invocation once this session's MCP connector re-syncs its tool list.
