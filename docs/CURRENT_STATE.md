...
3 write tools complete the `work_items` (next-action) lifecycle, all requiring `confirmed: true` and a caller-supplied `idempotencyKey`, all performing a canonical re-read before reporting `VERIFIED_SAVED` (never `SAVED` from the write response alone), all refusing idempotency-key reuse with a changed payload:
