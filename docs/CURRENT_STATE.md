...
### Stage Presence MCP — OAuth-backed Supabase Edge Function, 14 tools (2026-09-21)
...
9 read tools mirror the capability registry's read surface: `find_contact`, `find_engagement`, `get_pricing`, `generate_lead_summary`, `generate_job_sheet`, `search_stage_presence`, `get_attention_items`, `get_upcoming_engagements`, `build_quote_draft` (proposes but never persists a quote).

5 write tools, all requiring `confirmed: true` and a caller-supplied `idempotencyKey`, all performing a canonical re-read before reporting `VERIFIED_SAVED` (never `SAVED` from the write response alone), all refusing idempotency-key reuse with a changed payload:
