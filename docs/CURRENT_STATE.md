...
### Stage Presence MCP — OAuth-backed Supabase Edge Function, 20 tools (2026-09-21)
...
12 read tools: `find_contact`, `find_engagement`, `get_pricing`, `generate_lead_summary`, `generate_job_sheet` (now an operational projection — see below), `search_stage_presence`, `get_attention_items`, `get_upcoming_engagements`, `build_quote_draft` (proposes but never persists a quote), `find_team_member`, `find_resource`, `get_capabilities` (truthful `READ`/`DRAFT_ONLY`/`APPROVAL_REQUIRED`/`FRONTIER`/`NOT_IMPLEMENTED` list). Plus `generate_email` (draft-only, `readOnlyHint: true` — never sends, never changes engagement state).

7 write tools, all requiring `confirmed: true` and a caller-supplied `idempotencyKey`, all performing a canonical re-read before reporting `VERIFIED_SAVED` (never `SAVED` from the write response alone), all refusing idempotency-key reuse with a changed payload:
...
- `assign_team_member` — writes `engagement_assignments`, but the schema itself only permits `POSSIBLE`/`REQUESTED` — `CONFIRMED` is refused outright, since nothing in the schema evidences actual availability yet. Always reports that the assignment_state does not prove availability.
...
**Deployed**: server version `1.4.0` is live (Edge Function version 6, `ACTIVE`), confirmed byte-identical to `main` by re-fetching the deployed source and diffing/hashing it against the repo file (`sha256` match) immediately after deploy.
