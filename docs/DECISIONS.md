...
## 2026-09-20 — `mcp-bridge` gets its first write tool: `set_next_action`
Decision: add `set_next_action` as a 9th tool on the existing `stage-presence-mcp-bridge` Cloudflare Worker, using the same trust model already reviewed and shipped for the 8 read tools (a Worker-held `SUPABASE_SERVICE_ROLE_KEY`, never exposed to the Artifact or browser, running only fixed, hardcoded query/mutation shapes — no arbitrary SQL, no path for a caller to widen what gets touched). It writes exactly one `INSERT`-shaped upsert into `public.work_items` (schema per `database/20260909_reality_commercial_operations_kernel.sql` and `database/20260909_frontend_operating_primitives.sql`), keyed by a caller-supplied `idempotencyKey` via the table's existing unique `source_key` column so retries re-affirm the same row instead of duplicating it. It never modifies, cancels, completes, or reprioritizes any other existing work item, and only ever runs after `confirmed: true` is passed — which the Artifact must only send after the human clicked Approve on the exact proposed change. The tool always re-reads the row it just wrote (and the engagement's current `next_work`) before reporting `VERIFIED_SAVED`; a write that can't be confirmed by re-read is reported `NOT_SAVED`, never `SAVED`.
Reason: Greg explicitly asked for an OAuth/RLS-based Supabase connection for this write instead, but no such connection carries Stage Presence's narrow business-read/write surface — the only OAuth-based Supabase MCP in play is Anthropic's generic developer/management connector, which was already ruled out earlier as unsuitable for the Greg-facing production interface (it exposes `execute_sql`, project management, etc., not a fixed narrow surface). Introducing a real RLS/session-based write path would be new architecture (new auth flow, new deploy target), not the "finish the existing implementation" the task called for. Confirmed with Greg directly; he chose to keep the existing service-role-key trust model rather than open a second architecture decision mid-task.
