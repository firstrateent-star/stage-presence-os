...
## 2026-09-20 — `stage-presence-mcp` Supabase Edge Function gets its first write tool: `set_next_action`
Decision: add `set_next_action` as a 9th tool on the OAuth-backed `supabase/functions/stage-presence-mcp/index.ts` Edge Function, using the exact same trust model already shipped for its 8 read tools — the per-request Supabase client injected by `withSupabase({ auth: 'user' })`, scoped to the calling user's own session and subject to normal RLS (`work_items_member_all`). No service-role key, no raw SQL, no RLS bypass. It writes exactly one `INSERT` into `public.work_items` (schema per `database/20260909_reality_commercial_operations_kernel.sql` and `database/20260909_frontend_operating_primitives.sql`), keyed by a caller-supplied `idempotencyKey` via the table's existing unique `source_key` column. Idempotency is strict: the same key with an identical payload returns the already-saved, canonically re-read record; the same key with a *different* payload is refused (`NOT_SAVED`) rather than silently overwriting the earlier proposal — an idempotency key names one proposed mutation, not a mutable slot. It never modifies, cancels, completes, or reprioritizes any other existing work item, and only ever runs after `confirmed: true` (enforced by the input schema itself via `z.literal(true)`, not just application logic). It always re-reads the row it wrote (and the engagement's current `next_work` from `engagement_summary_v`) before reporting `VERIFIED_SAVED`; anything that can't be confirmed by that re-read is reported `NOT_SAVED`, never `SAVED`.
Reason: this supersedes the separate approach explored on `claude/confident-sagan-5y3701` (a Cloudflare Worker `mcp-bridge/` using a service-role key), which is not the branch this write capability belongs on per Greg's direction — the real, currently-deployed production MCP surface for Stage Presence is this Supabase Edge Function with Supabase OAuth 2.1, not the Cloudflare bridge. `mcp-bridge/` was left untouched.

## 2026-09-21 — Work-item lifecycle completed: `complete_next_action` and `update_next_action`
Decision: add two more write tools to `stage-presence-mcp` (server version bumped to `1.1.0`), completing the `work_items` lifecycle around `set_next_action`. Named `complete_next_action`/`update_next_action` (matching `set_next_action`'s established "next action" naming) rather than a generic `complete_work_item`/`update_work_item`, because no such generic naming exists anywhere else in the codebase — the frontend only ever displays `next_work` read-only, and `canonicalWrites.ts`'s only write path is `saveCanonicalNextMove`. Both target one specific `workItemId` (never "whatever is next"), never touch any other row, and use the same `withSupabase({ auth: 'user' })` client — no service-role key, no RLS bypass.
- `complete_next_action`: only allows `OPEN`/`WAITING`/`BLOCKED` → `DONE`; refuses on any other current status (including already-`DONE`, unless it's an exact idempotent replay of the same prior request). Sets `completed_at`.
- `update_next_action`: patches only the fields explicitly supplied (`title`, `actionType`, `priority`, `dueDate`, `whyNow`, `instructions`, `successCondition`); omitted fields are left untouched. Refuses to edit a closed (`DONE`/`CANCELLED`) item.
- Neither introduces a new table for idempotency bookkeeping: each row's own `metadata` jsonb column (already part of the schema) records which `idempotencyKey` produced its current state, so a replay with the same key is a safe no-op and the same key with a different payload is refused (`NOT_SAVED`) — the same idempotency discipline as `set_next_action`, reusing existing schema rather than adding new columns/tables.
- Both perform a canonical re-read (and re-read `engagement_summary_v.next_work`) before reporting `VERIFIED_SAVED`.
Reason: this is the safest, most concrete next increment identified for the work-item loop — no new business policy, no schema change, pure completion of a lifecycle the schema already fully supports. Contract tests added (`scripts/stagePresenceMcp.contract.test.mjs`, wired into `test:backend-contracts`) verify tool registration, annotations, confirmation gating, transition guards, and idempotency behavior structurally against the deployed source; the real Deno runtime can't be executed in this environment, so these are text-contract tests in the same style as `capabilityRegistry.contract.test.mjs`, not live-execution tests. No live business data was mutated to build or test this.

## 2026-09-21 — `create_lead`: the first real Stage Presence MCP intake path
Decision: add `create_lead` as a 12th `stage-presence-mcp` tool (server version → `1.2.0`). A lead is not a separate entity: this writes one `engagements` row (default `commercial_state`/`commitment_state` left untouched — the DB's own defaults apply, never overridden by this tool) plus, only where the caller supplies them, a linked contact `Party`/`engagement_parties` row, a `Location`/`engagement_locations` row, `engagement_facts` rows for explicitly-supplied "known unknowns" (`certainty_state: 'UNKNOWN'`, the state the schema already carries for exactly this purpose), and one initial `OPEN`/`FOLLOW_UP` `work_items` row if a next-action title is supplied.
- **Contact/venue matching mirrors `src/lib/canonicalWrites.ts`'s existing `linkCanonicalVenue` precedent exactly**: reuse an existing `Party`/`Location` only on an exact, unambiguous match (email or phone for contacts; exact name for venues); on more than one match, never guess — record the ambiguity as a `CONFLICTING` `engagement_facts` row and leave the link unset; on no match, create a new record. A bare contact name with no email/phone is never used to auto-match an existing Party (name alone isn't confident enough), only to label a newly created one.
- **Idempotent via `engagements.source_key`** — the same column the schema already reserves for "idempotent external-system imports" (Goodshuffle uses the `goodshuffle:` prefix there; this uses `mcp:create_lead:<idempotencyKey>`, which is a distinct prefix that the existing `.startsWith('goodshuffle:')` checks in `src/lib/businessPresentation.ts`/`decisionResolver.ts` correctly do not match). Because `engagements` itself carries no `metadata` column, the payload-signature check that catches idempotency-key reuse with a *different* payload lives on the linked `source_artifacts.metadata` row instead (the same provenance table `src/lib/repository.ts`'s `createEngagement` already writes for exactly this purpose) — no schema change.
- Always re-reads the created engagement from `engagement_summary_v` before reporting `VERIFIED_SAVED`, and reports which contact/venue-matching path was actually taken (`REUSED_EXISTING` / `CREATED_NEW` / `AMBIGUOUS_NOT_LINKED` / `NOT_PROVIDED`) rather than a bare success flag.
Reason: this is the minimum useful, fully schema-supported intake path the task called for — no new table, no invented default, no silent contact/venue merge, and dedup ambiguity handled by the same pattern the codebase already established rather than a new one. Contract tests extended in `scripts/stagePresenceMcp.contract.test.mjs` (structural, same limitation as above — Deno can't run in this environment). No live business data was mutated to build or test this.

## 2026-09-21 — `build_quote_draft` / `save_quote_draft`: the commercial proposal loop, without granting AI pricing authority
Decision: add two tools to `stage-presence-mcp` (server version → `1.3.0`).
- **`build_quote_draft`** (read-only, no persistence): matches an engagement's linked resources against the current Price Book and computes a proposed line-by-line total, marking each line's `authority_state` (`APPROVED_AUTHORITY` vs `DRAFT_CANDIDATE`) and flagging any resource with `needsPrice: true`. Nothing is written.
- **`save_quote_draft`** (write): persists a `DRAFT` `commercial_documents` + `commercial_document_lines` quote — only after the human approves the exact line list (normally the reviewed output of `build_quote_draft`). This is a faithful line-by-line port of the already-proven, already-shipped validation and calculation logic in `src/lib/pricingRuntime.ts` (`createDraftQuote`/`addQuoteLineFromPriceRule`/`addManualQuoteLine`/`recalculateQuoteTotals`) — no new pricing policy or arithmetic was invented. Specifically preserved from that precedent: a retired Price Book rule is rejected; a `DRAFT` (unapproved) rule requires an explicit `allowDraft`; only `BASE_RATE` rule kinds apply directly (discounts/surcharges stay explicit `MANUAL` lines); an `APPROVED` rule's effective-date window is enforced; `PERCENT` billing is rejected as a base line calculation; a proposed price that differs from the Price Book rate requires an explicit `priceAdjustmentReason`; a negative total is only permitted on a `DISCOUNT` line; and the exact totals formula from `recalculateQuoteTotals` (any negative `line_total` counts toward `discount_total`, not only `DISCOUNT`-typed lines).
- **Never promotes a `DRAFT_CANDIDATE` Price Book rate to approved policy** — it only snapshots which authority state was in effect at the moment of the quote (`pricing_rules.status` is never written by this tool).
- **Respects the existing "one current DRAFT quote per engagement" rule** from `createDraftQuote` rather than creating a duplicate; a retry that matches the current DRAFT's own `idempotencyKey`/payload replays idempotently, a different payload against an existing DRAFT is refused (`NOT_SAVED`).
- **Every line is validated before anything is written** — a two-pass design (validate all lines, then insert the document, then insert all lines, then recalculate totals) so one invalid line aborts the whole call rather than leaving a partial quote, which is a stricter safety property than the original per-line UI flow needed.
- Always re-reads the quote and its lines from `commercial_line_pricing_v` (the same enriched view the SPA already uses) before reporting `VERIFIED_SAVED`.
Reason: this is exactly the boundary the task drew — build the full commercial proposal loop, but never let the AI decide or promote pricing authority. Because the calculation and validation rules already exist, tested, in production (`pricingRuntime.ts`), the correct and safest approach was porting them verbatim rather than re-deriving quote math from scratch in a second language/runtime, which is exactly the kind of duplicated-economic-truth risk `docs/DECISIONS.md`'s 2026-09-12 Estimate Runtime entry already warned against. Contract tests extended in `scripts/stagePresenceMcp.contract.test.mjs` (structural). No live business data or real quote was created to build or test this.

## 2026-09-21 — `assign_team_member`, `add_resource_requirement`, an upgraded `generate_job_sheet`, `generate_email`, and `get_capabilities`: as far as the schema safely allows without confirmed availability, inventory holds, or sending mail
Decision: add five more tools to `stage-presence-mcp` (server version → `1.4.0`) plus two new read tools (`find_team_member`, `find_resource`) to resolve real people/equipment before proposing anything against them.
- **`assign_team_member`**: writes an `engagement_assignments` row, but the input schema itself only permits `assignment_state: 'POSSIBLE' | 'REQUESTED'` — `CONFIRMED`/`DECLINED`/`COMPLETED`/`UNKNOWN` are refused outright, because nothing in the schema (no calendar, no acknowledgement log) provides evidence a person is actually available. The response always carries an explicit `availabilityNote` stating this proposal does not prove or confirm availability. Idempotent via `engagement_assignments.source_key` (already a real unique column on that table).
- **`add_resource_requirement`**: writes only to `engagement_resources` (the existing `relationship` ∈ `CUSTOMER_REQUESTED/CONSIDERING/RECOMMENDED/CONFIGURED` + `required_from_date`/`required_through_date`/`requirement_window_state`/`planned_sourcing_model` fields the schema already carries for exactly this purpose) — it never writes to `resource_commitments` (the real `HOLD`/`RESERVATION`/`ALLOCATION` table, confirmed to exist cleanly but deliberately left untouched). The response always states this "does not mean this resource is available, held, or reserved." `engagement_resources` has no `metadata` column, so idempotency reuses the same `source_artifacts` provenance pattern `create_lead` established, backed by the table's own real `unique(engagement_id, resource_id, relationship)` constraint as an independent second guard against duplicates.
- **`generate_job_sheet`** (upgraded, not a new tool): now projects client/primary contact, venue (via `engagement_locations`, newly added to the shared loader's select), schedule, team assignments (explicitly labeling anything other than `CONFIRMED` as "NOT CONFIRMED — proposal only"), resource requirements (explicitly labeled `reservedInventory: false`), resource commitments from `resource_commitments` shown in a **separate** section (only if any exist — never merged with requirements), facts bucketed by their real `certainty_state` (`VERIFIED`/`KNOWN`/`ESTIMATED`/`ASSUMED`/`CONFLICTING`/`UNKNOWN`), and the current next action from `engagement_summary_v.next_work`. Still an internal projection, still read-only.
- **`generate_email`**: draft-only, read-only (no persistence, `annotations: { readOnlyHint: true }`). Produces `{ intendedAudience, subject, body, factualInputsUsed, assumptions }` for 7 kinds (lead response, missing-info request, quote follow-up, quote cover, event confirmation draft, logistics request, internal handoff) from live canonical context. Never sends, never marks anything sent, never writes to `engagements` or anywhere else.
- **`get_capabilities`**: a static, read-only truth table reconciling what this MCP surface can actually do — `READ` / `DRAFT_ONLY` / `APPROVAL_REQUIRED` / `FRONTIER` / `NOT_IMPLEMENTED`. Explicitly lists `send_email`, `mark_invoice_or_payment_paid`, and `delete_or_void_business_record` as `NOT_IMPLEMENTED`, and `confirm_crew_availability`, `create_resource_hold_or_reservation`, and `approve_or_change_pricing_authority` as `FRONTIER` — so the system can truthfully answer what it can do without a UI or registry entry ever implying a capability that isn't actually there.
Reason: this is the full remaining safe scope identified before genuine policy boundaries — confirming availability, creating holds/reservations, and sending mail — none of which have an evidence source, an established hold policy, or an authorized external-send capability yet. Contract tests extended in `scripts/stagePresenceMcp.contract.test.mjs` (34 tests total for this file; structural, same Deno-can't-run-here limitation as prior entries). No live business data was mutated to build or test this.

## 2026-09-21 — Deploy `stage-presence-mcp` v1.4.0; prove `build_quote_draft`'s read logic against live data
Deployed the merged `main` (commit `20e14c6`, containing all of the above) to the live Supabase Edge Function. Verified fidelity by re-fetching the deployed source immediately after deploy and diffing/hashing it byte-for-byte against the repo file (`sha256` match; Edge Function version 6, `ACTIVE`) — the same verification method used for the prior v5 deploy at the start of this session.

Then attempted the live, read-only `build_quote_draft` proof this task called for. This session's connected `stage-presence` MCP tool list was a fixed snapshot taken when it connected, from before `build_quote_draft`/`save_quote_draft`/`find_team_member`/`find_resource`/`get_capabilities`/`generate_email`/`assign_team_member`/`add_resource_requirement` existed, so none of those tools could be invoked directly from this session (a connector-snapshot limitation, not a policy refusal — the tools are deployed and callable by any freshly-connected MCP client). Rather than skip the proof, `buildQuoteDraft()`'s exact read-only matching logic — match each linked resource's name against `price_book_v` via `ilike`, prefer a candidate with `authority_state = 'APPROVED_AUTHORITY'`, else fall back to the next candidate, and flag no-match as `needsPrice` — was run as the identical SQL directly against the live database (read-only; nothing written). Against `SP-000030` ("Beerfest 2026," the engagement with the most linked resources at 15): 14 of 15 resources matched no priced Price Book entry at all (`needsPrice: true` under the real logic); one (`Load-in`) matched a `DRAFT_CANDIDATE` flat-$200 rule. A system-wide count confirmed `price_book_v` currently has **zero** rows with `authority_state = 'APPROVED_AUTHORITY'` (61 `NO_PRICE_EVIDENCE`, 30 `CURRENT_REFERENCE_ONLY`, 10 `DRAFT_CANDIDATE`, 4 `HISTORICAL_ONLY`) — so no engagement anywhere in the system can currently produce an `APPROVED_AUTHORITY` quote line, and `build_quote_draft`'s logic would correctly report that rather than fabricate one. `save_quote_draft` was not called. The tool itself still needs a direct invocation once this session's connector re-syncs its tool list against the newly-deployed manifest.

## 2026-09-21 — Phase 8–11: pricing governance, team/resource evidence layers, and a daily brief (server version → `1.5.0`)
With zero `APPROVED_AUTHORITY` pricing rules confirmed live, pricing governance was the principal remaining blocker to an operational quote loop. Six tools added, all built from schema that already existed (`pricing_rules.status/approved_by/approved_at`, `price_book_v.authority_state`, `engagement_assignments`, `engagement_schedule_items`, `resource_commitments`, `resources.quantity_state`) — no new pricing policy, availability policy, or hold policy was invented.

- **`get_pricing_review_queue`** (read): groups `price_book_v` by its own `authority_state` and adds a plain-language `reason` per item (e.g. "Drafted but not yet approved," "No pricing rule exists for this resource; only a current reference price is on file"). For `RULE`-kind rows it also joins `pricing_rules.updated_at` directly (`price_book_v` doesn't expose it), since that value is what `approve_pricing_rule`/`update_pricing_rule_draft` need as their stale-proposal guard.
- **`approve_pricing_rule`** (write, `APPROVAL_REQUIRED`): the one and only path from `DRAFT` to `APPROVED_AUTHORITY`. Writes only `status`/`approved_by`/`approved_at` (the schema's own approval fields, exactly matching `pricingRuntime.ts`'s already-proven `approvePricingRule`) plus this tool's idempotency key in the row's existing `metadata` column — never amount, percentage, or scope. Requires `expectedUpdatedAt` (from `get_pricing_review_queue`) and refuses if the live row's `updated_at` no longer matches — enforced both in application logic and as a second `.eq('updated_at', ...)` guard on the update itself, so a genuinely concurrent edit can't be silently overwritten. Refuses outright on a `RETIRED` rule; idempotently replays (no-ops) on a rule already `APPROVED` by this same proposal. Never reports `VERIFIED_SAVED` from the `pricing_rules` write alone — re-reads `price_book_v` for that `pricing_rule_id` and only confirms success if the view itself now reports `authority_state = 'APPROVED_AUTHORITY'`.
- **`update_pricing_rule_draft`** (write, `APPROVAL_REQUIRED`): edits name/amount/percentage/notes/rationale/effective window on one rule — refuses outright on any rule that isn't currently `DRAFT`, so an approved or retired rule's terms can never be silently changed by this tool. Same `expectedUpdatedAt` guard as `approve_pricing_rule`. Refuses `amount` on a `PERCENT`-rate rule and vice versa, matching the schema's own `pricing_rules_value_shape` constraint.
- **`check_team_conflicts`** (read): the evidence substrate the task explicitly asked for before ever building an availability policy. Compares a proposed window (explicit, or derived from an engagement's `engagement_schedule_items`/`event_start_date`) against a team member's other non-`DECLINED` assignments, reporting exactly one of `KNOWN_CONFLICTING_ASSIGNMENT` (exact time overlap), `KNOWN_SCHEDULED_ASSIGNMENT` (date-level-only overlap), `NO_CONFLICT_FOUND_IN_CANONICAL_DATA`, or `INSUFFICIENT_SCHEDULING_EVIDENCE` — every response carries an explicit disclaimer that "no conflict found" is never "available." `assign_team_member` now calls this internally and returns the result as `conflictEvidence` alongside every proposed `POSSIBLE`/`REQUESTED` assignment, without changing what states it's allowed to write.
- **`check_resource_pressure`** (read): the equipment equivalent. Compares recorded demand (other engagements' `engagement_resources` requirements and `resource_commitments`, split `TENTATIVE`/`CONFIRMED`) against a resource's configured `quantity` — but only treats that quantity as trustworthy when `quantity_state = 'VERIFIED'`; otherwise it says so explicitly (`INSUFFICIENT_EVIDENCE_TO_ASSESS`) rather than trusting an `UNVERIFIED`/`ESTIMATED`/`UNKNOWN` number. Never writes to `resource_commitments` and never reports `AVAILABLE` — its two positive states are named `DEMAND_WITHIN_CONFIGURED_QUANTITY`/`DEMAND_MAY_EXCEED_CONFIGURED_QUANTITY` specifically to avoid implying an availability determination this system doesn't have the evidence to make.
- **`generate_daily_brief`** (read): synthesizes `getAttentionItems`/`getUpcomingEngagements` (reused, not re-derived) plus overdue/`NOW` work items, open quotes, `engagement_pricing_position_v`'s own `pricing_readiness_state` for pricing blockers, `POSSIBLE`/`REQUESTED` staffing, resource requirements with zero `resource_commitments` evidence, `UNKNOWN`/`CONFLICTING` facts, and payment facts taken only from `engagement_summary_v.economy` (never inferred). Every synthesized line is tagged `FACT`, `DERIVED_ATTENTION`, or `UNKNOWN`. Mutates nothing.
- `get_capabilities`'s truth table updated: `approve_pricing_rule`/`update_pricing_rule_draft` are now `APPROVAL_REQUIRED`; the old `approve_or_change_pricing_authority` `FRONTIER` entry was narrowed to `create_pricing_rule` (creating a brand-new rule from scratch remains unbuilt; approving/editing an existing `DRAFT` rule is not).

Reason: this is exactly the schema-derivable scope the task asked for — pricing governance infrastructure, and evidence layers for team/resource conflict that deliberately stop short of an availability or hold policy. `confirm_crew_availability`, `create_resource_hold_or_reservation`, and creating a pricing rule from scratch remain true frontiers with no evidence source or approved policy behind them yet. Contract tests extended in `scripts/stagePresenceMcp.contract.test.mjs` (45 tests total; structural, same Deno-can't-run-here limitation as prior entries); full `test:backend-contracts` suite is 91/91. No live business data was mutated to build or test this.
